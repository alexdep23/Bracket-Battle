...
export default async function ArchiveTopicPage({
  ...
  params: { slug: string }; // NOTE: this "slug" is actually the topic ID now
}) {
  const supabase = supabaseAnon();

  // ✅ Look up topic by ID (no slug column needed)
  ...
  if (topicError) return <div className="p-6">Error: {topicError.message}</div>;
  if (!topic) return <div className="p-6">Not found.</div>;

  // ✅ Fetch matchups for this topic
  const { data: matchups, error: matchupsError } = await supabase
    .from("matchups")
    .select(
      "id,round,matchup_index,a_entry:entries!matchups_a_entry_id_fkey(id,name,seed),b_entry:entries!matchups_b_entry_id_fkey(id,name,seed)"
    )
  ...
  // Build counts map like: "matchupId:entryId" -> voteCount
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <BracketBoard
        currentRound={topic.current_round ?? 1}
        votingOpen={false} // ✅ archive is read-only
        votedMatchupIds={[]} // ✅ no voting state in archive mode
